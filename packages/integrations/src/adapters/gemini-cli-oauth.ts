/**
 * Gemini CLI OAuth utilities for marktoflow
 *
 * Ported from openclaw's google-gemini-cli-auth extension.
 * Extracts OAuth credentials from the installed gemini-cli binary,
 * and provides PKCE OAuth flow for Google authentication.
 */
...
import type { GeminiCliOAuthCredentials } from './gemini-cli-types.js';
...
// ============================================================================
// OAuth Context (for CLI interaction)
// ============================================================================
 ...
 * Finds the `gemini` binary in PATH, resolves symlinks, then reads the bundled
 * `@google/gemini-cli-core` oauth2.js to extract client ID and secret via regex.
 */
...
export async function discoverProject(accessToken: string): Promise<string> {
  ...
  const headers: Record<string, string> = {
    ...
    'X-Goog-Api-Client': 'gl-node/marktoflow',
  };
...
> {
  if (auth.api_key || auth.apiKey) {
    return {
      mode: 'api_key',
      apiKey: (auth.api_key || auth.apiKey) as string,
    };
  }

  if (auth.refresh_token || auth.refreshToken) {
    return {
      mode: 'oauth',
      refreshToken: (auth.refresh_token || auth.refreshToken) as string,
      projectId: (auth.project_id || auth.projectId || '') as string,
      clientId: (auth.client_id || auth.clientId) as string | undefined,
      clientSecret: (auth.client_secret || auth.clientSecret) as string | undefined,
    };
  }
