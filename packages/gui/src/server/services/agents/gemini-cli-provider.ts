/**
 * Google Gemini CLI AI agent provider
 * Uses Gemini API via OAuth (gemini-cli) or API key for AI capabilities
 */
...
export class GeminiCliProvider implements AgentProvider {
  ...
  async initialize(config: AgentConfig): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        this.ready = false;
        this.error = `Gemini API returned ${response.status}`;
      }
    } catch (err) {
      this.ready = false;
      this.error = 'Cannot connect to Gemini API. Check your API key or network.';
    }
  }
  ...
  async processPrompt(prompt: string, workflow: Workflow): Promise<PromptResult> {
    if (!this.ready || !this.apiKey) {
      return {
        explanation: 'Gemini is not available. Please set GEMINI_API_KEY.',
        error: this.error,
      };
    }
...
export function createGeminiCliProvider(config?: AgentConfig): GeminiCliProvider {
